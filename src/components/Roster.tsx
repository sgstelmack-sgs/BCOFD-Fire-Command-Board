 ...
 * - Features: Battalion/Station hierarchy, Medical mode toggles.
 * - Local Editing: Changes are stored in the card until 'SAVE UNIT' is pressed.
 * - Extra Person: Default role is "Extra" for quick entry.
 */
export default function Roster() {
  ...
  const [dirtyUnits, setDirtyUnits] = useState<string[]>([]); // Tracks which units need saving
  ...
  const addExtraMember = (unitId: string) => {
    const input = window.prompt(
      "Enter role name (or leave blank for 'Extra'):"
    );
    if (input === null) return;
    const roleName = input.trim() || "Extra";
  ...
  return (
    ...
    >
      <h2 style={{ marginBottom: "25px" }}>BCFD Command Roster</h2>
